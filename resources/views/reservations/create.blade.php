<x-app-layout>
    <div class="max-w-2xl mx-auto py-8 px-4">
        ...
        <form action="{{ route('reservations.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf
            <div>
                ...
                <select name="flight_id" id="flight_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    @foreach(App\Models\Flight::all() as $flight)
                        <option value="{{ $flight->id }}">{{ $flight->code }} - {{ $flight->origin }} → {{ $flight->destination }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label for="passengers" class="block text-sm font-medium text-gray-700">Passageiros</label>
                <input type="number" name="passengers" id="passengers" min="1" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="total_price" id="total_price" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>
